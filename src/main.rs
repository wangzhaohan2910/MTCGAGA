...
async fn main() {
    ...
    axum::serve(
        ...
        Router::new()
    .route("/", post(async move |Form(Frm {choice, text})| {
        match &choice[..] {
            "cmd" => {
        ...
        Html(
            ...
            r#"<!DOCTYPE html><html><body><form action="/" method="post"><select name="choice"><option value="cmd">命令</option><option value="code">键盘码</option><option value="eni">enigo按键</option><option value="rdev">rdev按键</option><option value="txt">输入</option><option value="clr">清空</option></select><input name="text"/><input type="submit"/></form><pre style="white-space:pre-wrap;">"#, )
                ...
                + r#"</pre><img src="screen.bmp"/><script>const ws=new WebSocket('screen.bmp');ws.onmessage=(e)=>{if(e.data instanceof Blob)document.querySelector('img').src=URL.createObjectURL(event.data)}</script></body></html>"#
        )))
            .route("/screen.bmp", get(async |ws: WebSocketUpgrade|
                ws.on_upgrade(async |mut sk| loop {
                    let mut buf = Cursor::new(Vec::new());
                    Monitor::all().unwrap()[0].capture_image().unwrap().write_to(&mut buf, ImageFormat::Bmp).unwrap();
